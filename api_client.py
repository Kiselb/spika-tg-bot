...
from config import AUTH_BASE_URL, API_BASE_URL, BOT_SECRET_KEY
...
class ApiClient:
    ...
    async def telegram_login(self, telegram_id: int, telegram_nick: str) -> Dict[str, Any]:
        """Возвращает ответ API в виде словаря (status, data)."""
        session = await self.get_session()
        url = f"{AUTH_BASE_URL}/Authentication/telegram-login"
        payload = {
    ...
    async def create_survey(self, token: str) -> Optional[int]:
        """Создаёт опрос и возвращает survey_id."""
        session = await self.get_session()
    ...
    async def get_survey(self, token: str, survey_id: int) -> Optional[Dict[str, Any]]:
        """Получает структуру опроса (GET, как было исправлено)."""
        session = await self.get_session()
    ...
    async def submit_answer(
        self, token: str, survey_id: int, question_id: int, answer_text: str
    ) -> bool:
